...
1. Set up a folder that wil contain all of Airflow's own stuff and assign it to the `AIRFLOW_HOME` environment variable.
2. Run `airflow config list --defaults > "${AIRFLOW_HOME}/airflow.cfg"`
3. Define `AIRFLOW__CORE__DAGS_FOLDER` as `${AIRFLOW_HOME}/dags` and create the folder. Copy the DAGs inside `flocs_processing/dags` to this folder.
4. Define `AIRFLOW__CORE__LOAD_EXAMPLES` as `False`
